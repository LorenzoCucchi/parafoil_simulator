...
void my_system(const state_type &x, state_type &dxdt, const double t){
    ...
    B.block<3,1>(0,0) =  Fa_w(vel_p)+W_w(euler)+W_b(euler) + Fa_b(vel_b) - Sk_Om*Mf*vel_p - A1*Sk_Om*vel_c + SFa(vel_p,sig(0))*sig;
    B.block<3,1>(3,0) = Ma_w(vel_p,vel_rot,euler)-Omrot(vel_p)*Mf*vel_p + Rgp*Fa_w(vel_p) - Rgp*Sk_Om*Mf*vel_p + Rgb*Fa_b(vel_b) - Sk_Om*(I+If)*vel_rot + (SMa(vel_p,sig(0))+Rgp*SFa(vel_p,sig(0)))*sig;
